...
export default function CtaStrip() {
  ...
  const handleEnrollClick = (e: React.MouseEvent) => {
    // If #inquiry-form exists on the current page (e.g. Home page), smooth scroll to it
    const formEl = document.getElementById("inquiry-form");
    if (formEl) {
      e.preventDefault();
      formEl.scrollIntoView({ behavior: "smooth", block: "start" });
    } else {
      // If on Academics, Campus, or Contact page without the inline form, open the Admission Modal!
      e.preventDefault();
      setIsModalOpen(true);
    }
  };
  ...
  return (
    <>
        ...
        <div className="max-w-[1200px] mx-auto px-4 lg:px-6 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <a
              ...
              className="px-6 py-3 rounded-full bg-primary-container text-on-primary font-title-md text-title-md hover:bg-primary shadow-sm hover:shadow-md hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2 cursor-pointer"
            >
      ...
      {/* Quick Enrollment Modal popup when clicked from any page */}
